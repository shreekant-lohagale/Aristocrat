...
export function EditorialCategories() {
  return (
    <section className="editorial-categories" aria-labelledby="editorial-categories-title">
      <header className="editorial-categories__heading"><p>02 / Shop by category</p><div><h2 id="editorial-categories-title">The House <em>Categories</em></h2><span aria-hidden="true" /></div></header>
      <div className="editorial-categories__grid">
        {editorialCategories.map((category) => <article className={`editorial-category editorial-category--${category.handle}${category.featured ? ' editorial-category--featured' : ''}`} key={category.id}>
