...
export function CatalogGrid({ collection, limit = pageSize, variant = 'default' }: CatalogGridProps) {
  ...
  const priceOptions = maxPrice ? [
    [`0-${firstPrice}`, `Under ${formatPrice(firstPrice, products[0]?.currencyCode)}`],
    [`${firstPrice}-${secondPrice}`, `${formatPrice(firstPrice, products[0]?.currencyCode)} – ${formatPrice(secondPrice, products[0]?.currencyCode)}`],
    [`${secondPrice}-0`, `${formatPrice(secondPrice, products[0]?.currencyCode)}+`],
  ] as const : [];
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      ...
      if (values.sort === 'alphabetical') return a.title.localeCompare(b.title);
      return 0;
    });
  ...
  if (error) return <section className="catalog"><div className="catalog-load-error" role="alert"><p>{error} Please try again.</p><button className="button" type="button" onClick={() => setRequestKey((key) => key + 1)}>Retry</button></div></section>;

  return (
    <section className={`catalog ${variant === 'new-arrivals' ? 'catalog--new-arrivals' : ''}`}>
      <div className="catalog-toolbar">
        <button className="catalog-filter-trigger" type="button" aria-haspopup="dialog" onClick={() => setDrawerOpen(true)}>
          <SlidersHorizontal size={14} aria-hidden="true" /> Filter{activeFilterCount ? ` (${activeFilterCount})` : ''}
        </button>
        <span className="catalog-toolbar__results">Showing {rangeStart}–{rangeEnd} of {filtered.length} {filtered.length === 1 ? 'product' : 'products'}</span>
        <div className="sort-control">
          <label><span>Sort by</span><select suppressHydrationWarning value={values.sort} onChange={(event) => update('sort', event.target.value)}>{sortOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        </div>
      </div>
...
function FilterPanel({ values, categories, colors, sizes, priceOptions, update, hasFilters, clear }: { values: { category: string; price: string; size: string; color: string; availability: string }; categories: string[]; colors: string[]; sizes: string[]; priceOptions: readonly (readonly [string, string])[]; update: (key: string, value: string) => void; hasFilters: boolean; clear: () => void }) {
  return <><div className="filter-heading"><b>Filters</b>{hasFilters && <button type="button" onClick={clear}>Clear all</button>}</div>{categories.length > 1 && <Filter label="Category" value={values.category} options={categories} onChange={(value) => update('category', value)} />}{priceOptions.length > 0 && <Filter label="Price" value={values.price} options={priceOptions} onChange={(value) => update('price', value)} />}{sizes.length > 0 && <Filter label="Size" value={values.size} options={sizes} onChange={(value) => update('size', value)} />}{colors.length > 0 && <Filter label="Colour" value={values.color} options={colors} onChange={(value) => update('color', value)} />}<Filter label="Availability" value={values.availability} options={[["in-stock", "In stock"], ["out-of-stock", "Out of stock"]]} onChange={(value) => update('availability', value)} /></>;
}

function Filter({ label, value, options, onChange }: { label: string; value: string; options: readonly (string | readonly [string, string])[]; onChange: (value: string) => void }) {
  return <details className="filter-group" open><summary>{label}</summary><div>{options.map((option) => { const [optionValue, optionLabel] = Array.isArray(option) ? option : [option, option]; return <label key={optionValue}><input type="radio" name={label} checked={value === optionValue} onChange={() => onChange(value === optionValue ? '' : optionValue)} />{optionLabel}</label>; })}</div></details>;
}
