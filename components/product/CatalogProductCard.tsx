...
export function CatalogProductCard({ product }: { product: CatalogProduct }) {
  ...
  return (
    <motion.article className="catalog-card" variants={staggerItem} whileHover={reducedMotion ? undefined : { y: -2 }} whileTap={reducedMotion ? undefined : { scale: 0.995 }}>
      <div className="catalog-image">
        ...
        <motion.button type="button" className="heart-button" whileTap={reducedMotion ? undefined : { scale: 0.9 }} onClick={() => toggleWishlist(product.handle, productWishlistAliases(product))} aria-label={`Toggle ${product.title} wishlist`}>
          <Heart size={18} fill={saved ? 'currentColor' : 'none'} />
        </motion.button>
        <motion.button type="button" className="catalog-add-button" whileTap={reducedMotion ? undefined : { scale: 0.985 }} onClick={add} disabled={!purchasable || added} aria-label={`${buttonLabel}: ${product.title}`}>
          {buttonLabel}
        </motion.button>
      </div>
