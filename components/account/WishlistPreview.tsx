...
export function WishlistPreview({ limit = 4, showAll = false }: { limit?: number; showAll?: boolean }) {
  ...
  if (status === 'error' || wishlistStatus === 'error') return <AccountEmptyState icon={Heart} title="We couldn’t load your saved pieces" description={wishlistError ?? 'Please try again when the collection is available.'} ctaLabel="Try again" onCta={() => {
    if (status === 'error') setRequestKey((key) => key + 1);
