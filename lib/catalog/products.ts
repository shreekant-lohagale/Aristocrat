...
export async function getCollectionProducts(handle: string, country = 'IN', sort = 'featured'): Promise<CatalogProduct[]> {
  ...
  if (!hasShopifyStorefrontConfig()) return (await getFallbackCatalog()).filter((product) => normalized === 'new-arrivals' || product.collectionHandles.includes(normalized));
  ...
  if (normalized === 'new-arrivals') return getCatalog(country);
...
export async function searchCatalog(query: string, country = 'IN'): Promise<CatalogProduct[]> {
  ...
  if (!hasShopifyStorefrontConfig()) { const lowered = term.toLowerCase(); return (await getFallbackCatalog()).filter((product) => `${product.title} ${product.category} ${product.colors.join(' ')}`.toLowerCase().includes(lowered)); }
...
export async function getProduct(handle: string, country = 'IN') {
  if (!hasShopifyStorefrontConfig()) return (await getFallbackCatalog()).find((product) => product.handle === handle);
